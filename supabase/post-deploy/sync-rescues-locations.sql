...
-- This script synchronizes rescue and location data from the CSV source.
-- It only updates records when there are actual differences to minimize audit log noise.
-- 
-- Usage:
--   psql -U postgres -f supabase/post-deploy/sync-rescues-locations.sql
--
-- Data Source: supabase/data/rescues.csv
-- Based on ADCH (Association of Dogs and Cats Homes) member list

-- Create temporary table to hold CSV data
CREATE TEMP TABLE temp_rescues (
  name TEXT NOT NULL,
...
-- Load data from CSV (this would be done via COPY in production or via a script)
...
-- This ensures every rescue has at least one location entry
...
BEGIN
  SELECT COUNT(*) INTO rescue_count FROM dogadopt.rescues;
  SELECT COUNT(*) INTO location_count FROM dogadopt.locations;
  
  RAISE NOTICE 'Sync completed successfully!';
  RAISE NOTICE 'Total rescues in database: %', rescue_count;
  RAISE NOTICE 'Total locations in database: %', location_count;
END $$;

-- Clean up
DROP TABLE temp_rescues;
