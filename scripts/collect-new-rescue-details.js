...
/**
 * UK Charity Commission API Contact Details Collector (New Rescues Only)
 * 
 * This script collects official contact information for newly added rescue organizations
 * (those added on 2025-12-31) using the UK Charity Commission API.
 */
...
async function collectRescueDetails(rescue) {
  ...
  if (!rescue.charity_number) {
    console.log(`   ⚠️  No charity number - skipping API call`);
    return {
...
async function main() {
  ...
  const { data: rescues, error } = await supabase
    .from('rescues')
    .select('id, name, charity_number, region, website, phone, email, address, postcode')
    .in('name', NEW_RESCUES)
    .order('name');
  ...
  console.log(`Found ${totalRescues} newly added rescues\n`);
  ...
  for (const rescue of rescues) {
    try {
      ...
      results.push({
        ...
        db_phone: rescue.phone,
        db_email: rescue.email,
        db_address: rescue.address,
        db_postcode: rescue.postcode,
        db_website: rescue.website,
        api_phone: details.phone,
        api_email: details.email,
        api_address: details.address,
        api_postcode: details.postcode,
        api_website: details.website,
      });
      
      if (details.hasCharityNumber && details.phone) {
        successful++;
      } else {
        skipped++;
      }
  ...
  const csvHeader = 'Rescue Name,Charity Number,Region,DB Phone,DB Email,DB Address,DB Postcode,DB Website,API Phone,API Email,API Address,API Postcode,API Website\n';
  const csvRows = results.map(r => 
    [
      formatCsvValue(r.name),
      formatCsvValue(r.charity_number),
      formatCsvValue(r.region),
      formatCsvValue(r.db_phone),
      formatCsvValue(r.db_email),
      formatCsvValue(r.db_address),
      formatCsvValue(r.db_postcode),
      formatCsvValue(r.db_website),
      formatCsvValue(r.api_phone),
      formatCsvValue(r.api_email),
      formatCsvValue(r.api_address),
      formatCsvValue(r.api_postcode),
      formatCsvValue(r.api_website),
    ].join(',')
  ).join('\n');
  
  const csvContent = csvHeader + csvRows;
  const outputPath = join(__dirname, '..', 'new-rescue-contacts.csv');
  ...
  console.log(`  ⚠️  Skipped (no charity number or no data): ${skipped}`);
  console.log(`\n📄 Results saved to: new-rescue-contacts.csv\n`);
}
